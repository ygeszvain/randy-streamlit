...
def bars_nmovies_imdb():
    ...
    ax.bar(annos, n_pelis, edgecolor="none",
           color=['#777', '#777', '#777', '#777', '#f5c518', '#f5c518', '#f5c518', '#f5c518', '#f5c518', '#f5c518',
                  '#777', '#444'])
    ...
    for anno, peli in tuple(zip(annos, n_pelis)):
        ax.text(anno, peli + 200, '{0:,}'.format(peli).replace(',', '.'), va='bottom', ha='center', fontsize=14,
                fontweight='regular');
...
def bars_nmovies(movies):
    ...
    for anno, peli in tuple(zip(nmovies.index.astype(int), nmovies.values)):
        ax.text(anno, peli + 10, '{0:,}'.format(peli).replace(',', '.'), va='bottom', ha='center', fontsize=18,
                fontweight='regular')
...
def heatmap_6x6(corr_6x6):
    fig = ff.create_annotated_heatmap(corr_6x6.round(2).to_numpy().tolist(),
                                      x=['Rating de IMDb', 'Metascore', 'Presupuesto', 'Recaudación', 'Beneficio',
                                         'ROI'],
                                      y=['Rating de IMDb', 'Metascore', 'Presupuesto', 'Recaudación', 'Beneficio',
                                         'ROI'],
...
def scatter_pointsvariable_economicvariable(movies, points_variable, economic_variable, title_points_variable,
                                            title_economic_variable):
...
def set_data():
    # customers = load_csv(path_articles)
    articles = load_csv(path_articles)
...
def set_otras_variables():
    ...
    if menu_otras_variables == "Géneros":
        ...
        st.markdown(
            'La distribuicón por géneros de las palículas es similar en los años de estudio, con con tres 4 géneros que representan la gran mayoría de las películas: Acción, Drama, Comedia  y Biografías/Documentales.')
        st.write(stack_bar_genres(movies))
    elif menu_otras_variables == "Países":
        st.markdown('### Número de películas por país de origen')
        st.markdown(
            'Estados Unidos es el país con mayor número de producciones con 892 películas, seguido de Reino Unido con 185 y en tercer lugar se encuentra Francia con 98 películas producidas. También es de interés el mercado canadiense con 56 producciones. Finalmente, España se encuentra representada por 26 películas.')
...
def set_relations():
    ...
    if menu_relations == "Rating/Metascore":
        ...
        if menu_relations_budget == "Relación entre Rating y Presupuesto":
            st.write(scatter_pointsvariable_economicvariable(movies, 'ratingImdb', 'budget',
                                                             title_points_variable='Rating de IMDb (1-10)',
                                                             title_economic_variable='Presupuesto ($)'))
            st.markdown('### Presupuesto por rangos de Ratings IMDb')
            st.write(table_ratings_economicvariable(movies, 'budget'))
            st.write(strip_rating_economicvariable(movies, economic_variable='budget',
                                                   title_economic_variable='Presupuesto ($)'))
        elif menu_relations_budget == "Relación entre Metascore y Presupuesto":
            st.write(scatter_pointsvariable_economicvariable(movies, 'metascore', 'budget',
                                                             title_points_variable='Metascore (1-100)',
                                                             title_economic_variable='Presupuesto ($)'))
            st.markdown('### Presupuesto por rangos de Metascore')
            st.write(table_metascores_economicvariable(movies, 'budget'))
            st.write(strip_metascore_economicvariable(movies, economic_variable='budget',
                                                      title_economic_variable='Presupuesto ($)'))
        ...
        if menu_relations_profit == "Relación entre Rating y Beneficio":
            st.write(scatter_pointsvariable_economicvariable(movies, 'ratingImdb', 'profit',
                                                             title_points_variable='Rating de IMDb (1-10)',
                                                             title_economic_variable='Beneficio ($)'))
            st.markdown('### Beneficio por rangos de Ratings IMDb')
            st.write(table_ratings_economicvariable(movies, 'profit'))
            st.write(strip_rating_economicvariable(movies, economic_variable='profit',
                                                   title_economic_variable='Beneficio ($)'))
        elif menu_relations_profit == "Relación entre Metascore y Beneficio":
            st.write(scatter_pointsvariable_economicvariable(movies, 'metascore', 'profit',
                                                             title_points_variable='Metascore (1-100)',
                                                             title_economic_variable='Beneficio ($)'))
            st.markdown('### Beneficio por rangos de Metascore')
            st.write(table_metascores_economicvariable(movies, 'profit'))
            st.write(strip_metascore_economicvariable(movies, economic_variable='profit',
                                                      title_economic_variable='Beneficio ($)'))
        ...
        if menu_relations_roi == "Relación entre Rating y ROI":
            st.write(scatter_pointsvariable_economicvariable(movies[movies.roi < 30], 'ratingImdb', 'roi',
                                                             title_points_variable='Rating de IMDb (1-10)',
                                                             title_economic_variable='ROI'))
            st.markdown('### ROI por rangos de Ratings IMDb')
            st.write(table_ratings_economicvariable(movies, 'roi'))
            st.write(strip_rating_economicvariable(movies[movies.roi < 30], economic_variable='roi',
                                                   title_economic_variable='ROI'))
        elif menu_relations_roi == "Relación entre Metascore y ROI":
            st.write(scatter_pointsvariable_economicvariable(movies[movies.roi < 30], 'metascore', 'roi',
                                                             title_points_variable='Metascore (1-100)',
                                                             title_economic_variable='ROI'))
            st.markdown('### ROI por rangos de Metascore')
            st.write(table_metascores_economicvariable(movies, 'roi'))
            st.write(strip_metascore_economicvariable(movies[movies.roi < 30], economic_variable='roi',
                                                      title_economic_variable='ROI'))
...
def set_arrays():
    ...
    if menu_arrays == "6x6":
        ...
        movies = primaryGenre(movies)
        corr_genre = movies[(movies.roi < 30) & (movies.primaryGenre == menu_genre)][
                         ['ratingImdb', 'metascore', 'budget', 'grossWorld', 'profit', 'roi']].corr().loc[
                     'ratingImdb': 'metascore', 'budget':'roi']
        st.write(heatmap_2x4(corr_genre))

        st.write('### Descriptivos para el Rating de usuarios según los géneros')
        movies['primaryGenre'] = movies['primaryGenre'].apply(grouppingGenres)
        rating_genres = movies.groupby('primaryGenre').ratingImdb.describe().T[
            ['Action', 'Adventure', 'Animation', 'Bio-Documentary',
             'Comedy', 'Drama', 'Horror', 'Thriller']]
        st.table(rating_genres)

        st.write('### Descriptivos para el Metascore según los géneros')
        metascore_genres = movies.groupby('primaryGenre').metascore.describe().T[
            ['Action', 'Adventure', 'Animation', 'Bio-Documentary',
             'Comedy', 'Drama', 'Horror', 'Thriller']]
        ...
        movies['primaryCountry'] = movies['countries'].apply(first_elem_csv)
        corr_country = movies[(movies.roi < 30) & (movies.primaryCountry == menu_country)][
                           ['ratingImdb', 'metascore', 'budget', 'grossWorld', 'profit', 'roi']].corr().loc[
                       'ratingImdb': 'metascore', 'budget':'roi']
        st.write(heatmap_2x4(corr_country))

        st.write('### Descriptivos para el Rating de usuarios según los países de origen')
        rating_countries = movies.groupby('primaryCountry').ratingImdb.describe().T[
            ['United States', 'United Kingdom', 'France', 'Canada', 'China', 'Spain']]
        st.table(rating_countries)

        st.write('### Descriptivos para el Metascore según los países de origen')
        metascore_countries = movies.groupby('primaryCountry').metascore.describe().T[
            ['United States', 'United Kingdom', 'France', 'Canada', 'China', 'Spain']]
